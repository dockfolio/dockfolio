...
~~codewithrigor.com~~ ✅ **VERIFIED 2026-06-29**, ~~thecreativeprogrammer.dev~~ ✅ **VERIFIED 2026-06-29** (both now collecting data). Still unverified: game subdomains creatureforge / diplomancy / grimhollow / huntingdragons / lufthafen / orb / urlgame / worldcontrol .crelvo.dev, and an unverified `sc-domain:abschlusscheck.de` duplicate (harmless; URL-prefix version is verified).

**Verification method that worked (repeatable, no creds, no DNS):** Google's HTML-file token is **per-account** — the single file `google3961c4e5a481bc42.html` (content: `google-site-verification: google3961c4e5a481bc42.html`) verifies ANY property for kreyhe12@gmail.com. Steps: in GSC click "Inhaberschaft bestätigen" → keep HTML-file method → `ssh deploy@VM`, write that file into the site's nginx `root` (find via `grep root /home/deploy/nginx-configs/sites/<name>`) → click "Bestätigen". Webroots seen: codewithrigor `/home/deploy/sites/codewithrigor`, creativeprogrammer `/opt/creativeprogrammer`. For reverse-proxied apps (Next.js etc.) add an nginx `location = /google3961c4e5a481bc42.html` returning the token instead.
