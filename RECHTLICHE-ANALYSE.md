...
- **betpilot.crelvo.dev TLS-Zertifikat abgelaufen + Auto-Renewal gefixt** → ✅ ERLEDIGT (Session 2026-06-17). Telegram-SSL-Alert „Certificate not trusted, expires in -4 days". **Ursache:** betpilot ist das einzige Zertifikat, das von einer **deploy-lokalen certbot-Instanz** (`--config-dir /home/deploy/certbot/config`, nginx zeigt auf `/home/deploy/certbot/config/live/betpilot.crelvo.dev/`) verwaltet wird, NICHT vom System-certbot unter `/etc/letsencrypt`. Der System-`certbot.timer` erneuert nur die `/etc/letsencrypt`-Lineages — die deploy-lokale Instanz hatte **gar keinen Renewal-Cron**, nur einen täglichen `nginx -s reload` (5:00). Folge: Zertifikat lief 90 Tage nach Ausstellung (15.03.) am **13.06.** ab, nichts erneuerte es. Die ACME-Challenge (Port 80, `root /home/deploy/acme-challenge`) liegt korrekt VOR dem `auth_basic` des 443-Blocks → die Privatisierung aus Session 25 war NICHT die Ursache. **Fix:** (a) sofort erneuert via `certbot renew --cert-name betpilot.crelvo.dev` mit den deploy-lokalen `--config-dir/--work-dir/--logs-dir` (gültig **17.06.→15.09.2026**), nginx reload, live verifiziert (`/`=401 privat, `/health`=200, Cert frisch); (b) fehlenden Renewal-Cron im **deploy-crontab** ergänzt: `30 3 * * * /usr/bin/certbot renew --config-dir /home/deploy/certbot/config --work-dir … --logs-dir … --quiet --deploy-hook "sudo nginx … -s reload" >> /home/deploy/logs/certbot-renew.log`. **Portfolio-Sweep (27 Domains, extern via openssl):** alle gültig, nächste Erneuerung 35 Tage entfernt (Jul-22-Cluster = System-certbot, erneuert automatisch). Kein weiteres Zertifikat gefährdet. **⚠️ Hinweis für nächste Session:** `sudo cat/ls/tail` auf der VM ist NICHT passwortlos (nur nginx/systemctl/certbot) → solche Lese-Diagnosen liefern in BatchMode leer; deploy-eigene Pfade ohne `sudo` lesen.
